...
def broken_detector_sp(x1_num, x2_num, k_num): x1, x2, phi, k = sp.symbols('x1 x2 phi k')
    ...
    n = sp.symbols("n", integer=True) print(type(sp.imageset(sp.cos,sol).args[0].args[0].args[1]))
...
def main():
    ...
    indecies = np.r_[broken_top[0]:broken_top[1], 50:51]
    print(indecies)
    ...
    g2.imshow(f, cmap=plt.cm.Greys_r)
    g1.imshow(appf, cmap=plt.cm.Greys_r)
